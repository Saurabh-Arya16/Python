...
import keyboard  # Library for keyboard input
...
def read(column, characters):
    ...
    if GPIO.input(R1) == 1:
        input_code += characters[0]
        print(input_code)
        lcd.lcd_display_string(str(input_code), 2, 0)
    if GPIO.input(R2) == 1:
        input_code += characters[1]
        print(input_code)
        lcd.lcd_display_string(str(input_code), 2, 0)
    if GPIO.input(R3) == 1):
        input_code += characters[2]
        print(input_code)
        lcd.lcd_display_string(str(input_code), 2, 0)
    if GPIO.input(R4) == 1:
        input_code += characters[3]
        print(input_code)
        lcd.lcd_display_string(str(input_code), 2, 0)
    GPIO.output(column, GPIO.LOW)

# Check for system keyboard input
def check_keyboard():
    global input_code
    if keyboard.is_pressed("enter"):
        if input_code == secretCode:
            ...
            input_code = ""  # Reset after validation
    elif len(input_code) > 0 and keyboard.is_pressed("backspace"):
        input_code = input_code[:-1]

try:
    while True:       
        lcd.lcd_display_string("Enter your PIN:", 1, 0)
        check_keyboard()  # Listen for keyboard input
